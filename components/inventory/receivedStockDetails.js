...
$(async()=>{
    ...
    $('#srHead').text(poID)
    orderID = poID
    getPurchaseOrderDetail(billNumber)
})
...
function getPurchaseOrderDetail(billNumber) {
    db.all(`SELECT * FROM stockTransactionSummary WHERE billNumber = '${billNumber}'`,function(err, stockData) {
        if(err){
            console.log(err)
