...
handleChildProductBatchToCart = (fifoSBatchData,productID,type,action)=>{
    ...
    checkReviewRender = (selectedProductData.filter(a=>a.isChild == true)).length
...
assignBatchToCart=async (taxedSBatchData,productID,type,action)=>{
   ...
   if(action=='remove'){
       ...
       sProductBatchData.map(m=>{
        ...
        parentNoArr.map(l=>{
            sProductBatchData = sProductBatchData.filter(m=>m.serialNumber !=l)
            sProductData = sProductData.filter(m=>m.serialNumber !=l)
        })
...
reRenderTable=async (productID,cartData,selectedProductBatchData,selectedProductData,type)=>{
    ...
    checkReviewRender2 = (selectedProductData.filter(a=>a.isChild == true)).length
    if(checkReviewRender2>checkReviewRender){
        var reviewSectionDisplay = $('#reviewBillingCart').css('display')
        if(reviewSectionDisplay == 'flex'){
            proceedCart()
        }
